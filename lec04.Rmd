...
\[
\sum_{i=1}^n \left\lvert y_i - \vec{x}_i^\top \widehat\beta \right\rvert.
\]
...
So I use 
\[
\sum_{i=1}^n ( y_i - \vec{x}_i^\top \widehat\beta )^2.
\]
...
We write this as
\[
\widehat\beta = \arg\min_\beta \sum_{i=1}^n ( y_i - \vec{x}_i^\top \beta )^2.
\]
...
\[
\widehat\beta = \arg\min_\beta \frac{1}{n}\sum_{i=1}^n ( y_i - \vec{x}_i^\top \beta )^2.
\]
...
\[
\begin{aligned}
& \frac{\partial}{\partial \beta} \frac{1}{n}\sum_{i=1}^n ( y_i - \vec{x}_i^\top \beta )^2\\
&= \frac{2}{n}\sum_{i=1}^n \vec{x}_i (y_i - \vec{x}_i^\top\beta)\\
&= \frac{2}{n}\sum_{i=1}^n - \vec{x}_i \vec{x}_i^\top \beta + \vec{x}_i y_i\\
0 &\equiv \sum_{i=1}^n - \vec{x}_i \vec{x}_i^\top \beta + \vec{x}_i y_i\\
&\Rightarrow \sum_{i=1}^n \vec{x}_i \vec{x}_i^\top \beta = \sum_{i=1}^n \vec{x}_i y_i\\
&\Rightarrow \beta = \left(\sum_{i=1}^n \vec{x}_i \vec{x}_i^\top\right)^{-1}\sum_{i=1}^n \vec{x}_i y_i
\end{aligned}
\]
...
f_Y(y ; \beta) &= \prod_{i=1}^n f_{y_i ; \beta}(y_i)\\
  &= \prod_{i=1}^n \frac{1}{\sqrt{2\pi\sigma^2}} \exp\left(-\frac{1}{2\sigma^2} (y_i-\vec{x}_i^\top \beta)^2\right)\\
  &= \left( \frac{1}{2\pi\sigma^2}\right)^{n/2} \exp\left(-\frac{1}{2\sigma^2}\sum_{i=1}^n (y_i-\vec{x}_i^\top \beta)^2\right)
\end{aligned}
\]


We initially learn to think of $f_Y$ as a function of $y$ with $\beta$ fixed:
...
The derivative of $L(\beta)$ tractable but a pain to work with. (Why is it so bad?)

* If we're trying to maximize over $\beta$, we can take the $\log$ of $L(\beta)$, and maximize over the log function instead.
* We will get the same solution for $\beta$. Why?
* It will be easier too. Again... Why?


\[
\begin{aligned}
\mathcal{L}(\beta) &= \left( \frac{1}{2\pi\sigma^2}\right)^{n/2} \exp\left(-\frac{1}{2\sigma^2}\sum_{i=1}^n (y_i-\vec{x}_i^\top \beta)^2\right)\\
\ell(\beta) &=-\frac{n}{2}\log (2\pi\sigma^2) -\frac{1}{2\sigma^2} \sum_{i=1}^n (y_i-\vec{x}_i^\top \beta)^2
\end{aligned}
\]
...
\[
\widehat\beta = \arg\max_\beta -\sum_{i=1}^n (y_i-\vec{x}_i^\top \beta)^2
\]
...
\[
\widehat{\beta} = \arg_\beta \min \sum_{i=1}^n w_i \left( y_i - \vec{x}_i^\top \vec{\beta} \right) ^2
\]
...
3. Examples:
    ...
    * OLS Regression: $\widehat{y} = \vec{X}\widehat{\vec{\beta}} = X(\vec{X}\prime\vec{X})^{-1}\vec{X}\prime\vec{y}$
